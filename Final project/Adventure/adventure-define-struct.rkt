...
(define (add-type! name-string predicate)
  (set! type-name-list
        (cons (cons name-string predicate)
              type-name-list)))

(define (name->type-predicate string-name)
  (let ((probe (assoc string-name type-name-list)))
    (if probe
        (cdr probe)
        #f)))

(define (type-name-string object)
  (let ((probe (findf (λ (pair)
                        ((cdr pair) object))
                      type-name-list)))
    (if probe
        (car probe)
  ...
  (syntax-parse stx
    ...
    [(_ names:expr [fields:id ...] #:methods methods:def ...)
     (with-syntax* ([((prop (lifts ...)) ...)
       ...
       (let* ((name (if (identifier? #'names) #'names (car (syntax-e #'names))))
              (name-string (format "~A" (syntax-e name))))
       #`(begin
